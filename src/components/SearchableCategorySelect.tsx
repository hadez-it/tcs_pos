  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      <button
        ...
        className="w-full pl-9 pr-8 py-2.5 bg-white border border-slate-200 hover:border-gray-400 rounded-xl text-xs font-bold text-slate-800 focus:outline-none focus:border-gray-900 focus:ring-2 focus:ring-black/10 shadow-sm flex items-center justify-between cursor-pointer transition-all"
      >
        <Filter className="absolute inset-y-0 left-0 pl-3 w-4 h-4 my-auto text-gray-900 pointer-events-none" />
        ...
        <ChevronDown className={`absolute inset-y-0 right-0 pr-2.5 w-4 h-4 my-auto text-slate-400 pointer-events-none transition-transform duration-200 ${isOpen ? 'rotate-180 text-gray-900' : ''}`} />
      </button>
          ...
          <div className="p-2 border-b border-slate-100 bg-slate-50/80 sticky top-0">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-7 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium text-slate-900 focus:outline-none focus:border-gray-900"
                autoComplete="off"
              />
              {searchQuery && (
                <button
              ...
              {creatingNew && (
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-900 hover:text-gray-900 p-0.5 cursor-pointer"
                >
            ...
            {filteredOptions.length === 0 && !creatingNew ? (
              <div className="py-4 px-3 text-center text-xs text-slate-400 font-medium">
                ...
                {allowCreate && searchQuery.trim() !== '' && (
                  <div className="mt-2 text-xs font-medium text-gray-900 cursor-pointer hover:text-gray-900" onClick={() => {
                    onChange(searchQuery.trim());
                    setIsOpen(false);
                    setCreatingNew(false);
                    setSearchQuery('');
              ...
              <>
                      ...
                      <div className="flex items-center gap-1.5 ml-2 shrink-0">
                        {opt.count !== undefined && (
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                            isSelected ? 'bg-gray-100 text-gray-900' : 'bg-slate-100 text-slate-500'
                          }`}>
                            {opt.count}
                          </span>
                        )}
                ...
                {allowCreate && !creatingNew && searchQuery.trim() !== '' && (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg text-xs font-semibold flex items-center justify-between transition-all cursor-pointer text-gray-900 hover:bg-gray-50 hover:text-white"
                  >
                    <span className="truncate">
                      Create "{searchQuery}"
                    </span>
