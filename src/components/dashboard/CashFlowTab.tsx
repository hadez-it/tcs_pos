...
import { Search, Building2, Calendar, Layers, Filter } from 'lucide-react';
...
export default function CashFlowTab({
  ...
  const activeFilterCount = useMemo(() => {
    let count = 0;
    if (cfRange !== 'this_month') count++;
    if (cfTypeFilter !== 'all') count++;
    if (cfCategoryFilter !== 'All') count++;
    if (selectedBranchId !== 'all') count++;
    if (cfSearch.trim()) count++;
    return count;
  }, [cfRange, cfTypeFilter, cfCategoryFilter, selectedBranchId, cfSearch]);

  const resetFilters = () => {
    setCfRange('this_month');
    setCfStartDate('');
    setCfEndDate('');
    setCfTypeFilter('all');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-premium">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => setShowCfFilters(true)}
              className={`inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-bold rounded-xl border transition-all cursor-pointer shadow-2xs ${
                activeFilterCount > 0
      ...
      <FilterDrawer
        isOpen={showCfFilters}
        onClose={() => setShowCfFilters(false)}
        title="Cash Flow Filters"
        subtitle="Filter ledger by period, flow type & category"
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Period</label>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Flow Type</label>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            ...
            <select
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl py-2.5 px-3 font-medium text-slate-800 text-xs focus:outline-none focus:border-black focus:bg-white"
            >
